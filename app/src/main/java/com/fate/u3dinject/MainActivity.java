...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(actionBar!=null){
            if(isModuleActive()) {
                actionBar.setTitle("List<Unity3d>(Activated)");
            }else {
                actionBar.setTitle("List<Unity3d>(Not Activated)");
                Toast.makeText(this,"Current Module is not activated!",Toast.LENGTH_SHORT).show();
            }
        }

        Toast.makeText(this,"Please choose unity3d app to inject so!",Toast.LENGTH_SHORT).show();
        bindViews();
        getPackages();
    ...
    private void getPackages() {
        ...
        for (int i = 0; i < packages.size(); i++) {
            ...
            if ((packageInfo.applicationInfo.flags& ApplicationInfo.FLAG_SYSTEM) == 0) { //非系统应用
                ...
                String unityPath=packageInfo.applicationInfo.nativeLibraryDir+"/libunity.so";
                File monoSo=new File(unityPath);
                if(monoSo.exists()) {
                    appInfo.setAppName(
